    ...
    for filename in query_filenames:
        ...
        for i, result in enumerate(results):
            if result == label:
                ...
                break
        total[label] += 1
    for gloss in incorpus:
        for kv in [1, 5, 10, 20]:
            output.append([model_name, gloss, 'Seen', kv, n, (correct[gloss] / total[gloss])[kv-1]])
    for gloss in notincorpus:
        for kv in [1, 5, 10, 20]:
            output.append([model_name, gloss, 'Unseen', kv, n, (correct[gloss] / total[gloss])[kv-1]])
...
if __name__ == '__main__':
    ...
    outputs = [['Upstream language', 'Gloss', 'Group', 'k', 'N', 'Accuracy']]
    import tqdm
    for n in tqdm.tqdm([100, 200, 300, 400, 500, 1000, 5000, 10235]):
