...
def _process(video_file: str, output_directory: str, model_path: str, extension: str, face_first: bool,
             keypoints_preextracted: bool) -> bool:
    ...
    model: Model = Model(model_path, face_first)

    try:
        if not keypoints_preextracted:
            frames: np.ndarray = load_video(video_file)
            features: np.ndarray = preprocess_video(frames)
        else:
            features = np.load(video_file)
...
def main(input_directory: str, output_directory: str, model_path: str, extension: str, jobs: int, face_first: bool,
         keypoints_preextracted: bool):
    ...
    _results = Parallel(n_jobs=jobs)(
        delayed(_process)(video_file, output_directory, model_path, extension, face_first, keypoints_preextracted) for
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--face_first', action='store_true', help='Set this flag for Maxim\'s model.')
    parser.add_argument('-k', '--keypoints_preextracted', action='store_true', help='Load keypoints instead of videos.')

    args = parser.parse_args()

    main(args.input_directory, args.output_directory, args.model_path, args.extension, args.jobs, args.face_first,
         args.keypoints_preextracted)
